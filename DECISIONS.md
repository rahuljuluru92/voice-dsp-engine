...
**Live confirmation (2026-08-23, project owner, `pitch_up_third` preset, real hardware, 30s):** `callback_count=5644`, `underrun_count=68`, `underrun_samples=1202` (25.0ms total, 0.083% of the run) -- more individual underrun events than the previous chunked-crossfade version's 19-26, but each one much smaller (~17.7 samples/event vs. ~256 samples/event), consistent with this architecture releasing output in small per-analysis-hop increments rather than large chunks. `bypass_count` and both queue-dropped counts stayed 0. Audible result, reported directly by the project owner: clicking is "yes, a very little" -- a clear, substantial improvement from "clicks got worse" on the pre-rewrite chunked version. The remaining small amount of clicking is most plausibly explained by these brief real-time underrun events (genuine Python/OS scheduling jitter feeding the callback, not the phase-vocoder discontinuity this rewrite targeted, which the synthetic tests above show is fully eliminated) rather than a residual DSP correctness issue.
